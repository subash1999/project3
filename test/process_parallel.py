...
if __name__ == '__main__':   
    ...
    for i in range(beg,end,step):
        ...
        for i2 in range(i,(i+step),1):
            ...
            p = multiprocessing.Process(target=multiprocess,args=([matrix_cols[i2],normal_matrix,p_count]))
            processes.append(p)
            p.start()             
        for process in processes:
            process.join()
        print("*PROCESS *",i,"--",i+step," END")
        print("TIME REQ :::: ",time.time()-p_time)
        print("PER PROCESS :::::---",(time.time()-p_time)/step)
