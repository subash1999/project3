...
def get_relapse_value_from_series_matrix(series_matrix : pd.DataFrame)->list:
    relapse_array = []
...
def get_relapse_value(id_ref : str)->int:
...
def get_relapse_value_from_list(id_list : list)->list:
    l=[]
    for x in id_list:
        l.append(get_relapse_value(x))
    return l
